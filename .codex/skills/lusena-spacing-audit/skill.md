...
### MANDATORY: Use a unique named session
...
**Every `playwright-cli` command MUST include `-s=<unique-name>`.** Use a name that includes the page being audited: `-s=spacing-homepage`, `-s=spacing-pdp`, `-s=spacing-quality`, etc. Never use generic names like `-s=spacing-homepage` or `-s=test`.

### URL: Use the correct preview URL for your context

**Main repo (`lusena-dawn/`):** `http://127.0.0.1:9292/{page-path}` (if `shopify theme dev` is running)

**Worktree (`lusena-worktrees/lusena-N/`):** Push changes first, then use:
`https://lusena-dev.myshopify.com{page-path}?preview_theme_id=<THEME_ID>`
Look up your theme ID from `config/worktree-themes.json` (key = slot N). Store password: `paufro`.

Code examples below use `http://127.0.0.1:9292/` — **replace with your preview URL when in a worktree.**

```bash
# CORRECT — isolated session via Bash tool:
playwright-cli -s=spacing-homepage open http://127.0.0.1:9292/
playwright-cli -s=spacing-homepage resize 1440 900
playwright-cli -s=spacing-homepage eval "document.title"
playwright-cli -s=spacing-homepage run-code "async (page) => { ... }"
...
**Rule of thumb:**
- **Simple expressions** (< 1 line, no nested quotes) → `playwright-cli -s=spacing-homepage eval "..."`
- **Multi-step code** (addScriptTag + evaluate) → `playwright-cli -s=spacing-homepage run-code "async (page) => { ... }"`
...
```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight)); await page.waitForTimeout(1500); await page.evaluate(() => window.scrollTo(0, 0)); await page.waitForTimeout(500); await page.addScriptTag({ path: 'docs/spacing-audit/measure.js' }); await page.waitForTimeout(500); return await page.evaluate(() => window.__lusenaSpacingAudit ? JSON.stringify(window.__lusenaSpacingAudit.summary) : 'NO DATA'); }"
```
...
```bash
playwright-cli -s=spacing-homepage open http://127.0.0.1:9292/{page-path}
playwright-cli -s=spacing-homepage resize {width} {height}
```
...
```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.addScriptTag({ path: 'docs/spacing-audit/measure.js' }); }"
```

Verify it worked:
```bash
playwright-cli -s=spacing-homepage eval "window.__lusenaSpacingAudit ? window.__lusenaSpacingAudit.summary.totalSections + ' sections' : 'NO DATA'"
```
...
```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.addScriptTag({ path: 'docs/spacing-audit/inject-spec.js' }); }"
```
...
Verify:
```bash
playwright-cli -s=spacing-homepage eval "window.__lusenaSpacingSpec ? window.__lusenaSpacingSpec.page : 'NO SPEC'"
```

#### Step 4: Run comparison

```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.addScriptTag({ path: 'docs/spacing-audit/compare.js' }); }"
```
...
Summary:
```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingReport.summary)"
```

Full check details (if failures exist):
```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingReport.checks.filter(function(c){return c.status !== 'PASS'}))"
```

All checks:
```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingReport.checks)"
```

Bug list:
```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingReport.bugs)"
```
...
```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingAudit.summary)"
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingAudit.bugs)"
```

For the full gap map (all intra-section spacing):
```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.addScriptTag({ path: 'docs/spacing-audit/extract-all-gaps.js' }); await page.waitForTimeout(300); return await page.evaluate(() => window.__gapResult); }"
```

For a specific component's internal gaps (e.g., buybox, summary):
```bash
playwright-cli -s=spacing-homepage eval "window.__componentSelector = '.lusena-pdp-buy-box'"
```
then:
```bash
playwright-cli -s=spacing-homepage run-code "async (page) => { await page.addScriptTag({ path: 'docs/spacing-audit/extract-component.js' }); await page.waitForTimeout(300); return await page.evaluate(() => window.__componentResult); }"
```

#### Step 4: Read section-level data

```bash
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingAudit.sections.map(function(s){return {id:s.id, classes:s.classes, paddingTop:s.paddingTop, paddingBottom:s.paddingBottom}}))"
playwright-cli -s=spacing-homepage eval "JSON.stringify(window.__lusenaSpacingAudit.sectionGaps)"
```
...
The page might not have a `<main>` or `#MainContent` element, or it may have redirected. Check `playwright-cli -s=spacing-homepage eval "document.querySelector('main') ? 'found' : 'missing'"`.
...
Shopify hot-reload may navigate to a different page when CSS files are saved. After making CSS changes, re-navigate to the target page and re-inject `measure.js` from scratch. Always use `playwright-cli -s=spacing-homepage goto` to force a fresh page load.
...
Some CSS (e.g., Shopify's responsive JS) evaluates on page load. For mobile audits: resize first, then `playwright-cli -s=spacing-homepage goto` to reload the page at the correct viewport. Do NOT just resize after the page has loaded — you may get hybrid desktop/mobile measurements.
