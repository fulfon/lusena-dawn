---
name: lusena-spacing
description: 'Use when adjusting spacing in the LUSENA Shopify theme for spacing inside containers (content flow) and between sections (tier padding). Invoke for any spacing review, fix, or new section setup on any page. Keywords: spacing, padding, gap, margin, content-flow, tier, section padding, breathing room, vertical rhythm.'
---

# LUSENA spacing â€” page review & adjustment skill

## Goal

Review any page in the LUSENA theme and ensure its spacing â€” both **between sections** and **inside containers** â€” delivers the best possible UI/UX for a premium e-commerce store. Every spacing decision must serve two purposes:

1. **Clean, aesthetic, premium feel** â€” generous breathing room, consistent rhythm across all pages
2. **Maximize conversion** â€” guide the customer's eye naturally toward trust signals and CTAs
...
The user is open to new classes â€” just ask first.
...
| File | Purpose |
|---|---|
| `assets/lusena-spacing.css` | **Source of truth** â€” all tokens, tiers, gap utilities, content-flow utilities |
...
| `docs/theme-brandbook-uiux.md` (Â§3.3, Â§5.4) | Design documentation |
...
| Class | Mobile | Desktop | When to use |
|---|---|---|---|
| `lusena-spacing--snug-top` | 32px | 48px | Hero sections whose `bg-*` matches the site header (both `bg-brand-bg`). Reduces (not removes) top padding â€” uses compact-tier values for a "minimal entry, generous exit" rhythm. |

Combine with any tier class: `class="lusena-spacing--hero lusena-spacing--snug-top"`

**Why 32 / 48px (compact-tier values):**
- Reuses an existing spacing decision â€” keeps the scale coherent instead of inventing arbitrary values.
- 32px mobile â‰ 2Ă— the header's own internal vertical spacing feel.
...
**Design decision log:** Initially we tried `0px` (flush-top), but that made the kicker collide with the nav â€” no breathing room at all. Full hero-tier padding (64/96px) created a visible void since there's no color boundary. Compact-tier values (32/48px) are the sweet spot: clear separation without emptiness.

**When NOT to use:** If the hero has a different background than the header (e.g. `bg-white`, full-bleed image, gradient), the color boundary already provides visual separation â€” keep the normal tier padding.

**Currently applied to:** `lusena-quality-hero.liquid`, `lusena-returns-hero.liquid`.

**Note:** The section-gap-detector (`lusena-section-gap-detector.liquid`) only works between `<section>` elements â€” it cannot detect headerâ†’section same-bg. This modifier must be applied manually in Liquid.
...
| `lusena-content-flow` | 20px | 24px | **Default** â€” most containers |
| `lusena-content-flow--relaxed` | 28px | 32px | Visually heavy children (large images, card grids) |

#### Standard kicker+heading pattern

Every kicker â†’ heading (and heading â†’ subheading) pair **must** be wrapped in a `<div class="lusena-content-flow--tight lusena-gap-section-intro">`. This is the only sanctioned pattern:
...
**Never** use individual `lusena-gap-kicker` / `lusena-gap-heading` on the kicker or heading elements directly â€” see Rule 9.
...
> **Inline element warning:** `margin-top` / `margin-bottom` are **silently ignored** on inline elements (`<span>`, `<a>`, `<em>`, etc.). If you must use a gap class on a `<span>`, it **must** also have `display: block` (the CSS for `lusena-gap-kicker` includes this). But the preferred approach is to avoid individual gap classes on inline elements entirely â€” use a `lusena-content-flow--tight` wrapper instead (see standard kicker+heading pattern above).

| Class | Mobile | Desktop | Use case |
|---|---|---|---|
| `lusena-gap-kicker` | 12px | 12px | **Deprecated for kicker+heading pairs** â€” use `lusena-content-flow--tight` wrapper instead. Only for rare standalone kicker use outside a heading group. Has `display: block` built in. |
...
| `lusena-gap-cta-top` | 28px | 36px | Above a CTA that follows content (margin-top) â€” use when the CTA is conditionally hidden on some breakpoints (see Rule 15) |
...
# Hardcoded Tailwind spacing (should be zero â€” flag any you find)
...
1. **Section tier class** â€” does it match the section's conversion role?
2. **Container content-flow** â€” is it using a LUSENA content-flow class, or hardcoded Tailwind `space-y-*`?
3. **Individual element gaps** â€” are there hardcoded `mb-*`, `mt-*`, `pt-*` that should be LUSENA gap classes instead?
...
- What is this section's job? Hero â†’ `hero`. Trust/CTA â†’ `spacious`. Info â†’ `standard`. Utility â†’ `compact`.
- Does the padding feel generous and premium, or cramped?
- **CTA/conversion sections must always be `spacious` or `hero`** â€” the final CTA is the climax, never `compact`.
...
- Is there a kicker+heading pair? â†’ `lusena-content-flow--tight`
- Is it body content (heading â†’ paragraphs â†’ link)? â†’ `lusena-content-flow`
- Are there visually heavy blocks? â†’ consider `lusena-content-flow--relaxed`
...
3. Verify on `http://127.0.0.1:9292/` â€” check both desktop and mobile viewports
...
4. **Never mix content-flow with individual gap classes on the same container.** Content-flow sets `margin-top` on all children â€” adding `lusena-gap-heading` (`margin-bottom`) on a child creates double spacing.

5. **Standard content-flow (24px) is the default for 90% of containers.** Don't overuse `--relaxed` â€” it makes text feel disconnected.
...
11. **Never apply `margin-bottom` / `margin-top` gap classes to inline elements** (`<span>`, `<a>`, `<em>`) without ensuring `display: block`. Browsers silently ignore vertical margins on inline elements â€” the spacing will look correct in some conditions but fail in others. Prefer wrapping in a content-flow container instead.

12. **Same-background adjacent sections: gap-same is a floor, not a bonus.** The `lusena-section-gap-same` class (applied by the JS gap-detector) uses `max(gap-same, tier-pt)` â€” it ensures *at least* `--lusena-section-gap-same` (40px desktop / 32px mobile) of top padding, but never inflates sections that already carry generous tier padding. This prevents an empty void between same-bg sections whose own tiers already provide sufficient separation.

    **How it works:** `padding-top: max(var(--lusena-section-gap-same), var(--lusena-tier-pt))`. For `full-bleed` (0px tier) â†’ gap-same kicks in (40px). For `standard` (64px tier) â†’ tier already exceeds gap-same, so no change.

    **Why not additive:** An earlier version used `calc(gap-same + tier-pt)`, which inflated every same-bg pair: e.g. `spacious` (80px) + 40px = 120px bottom + 120px top = 240px void â€” far too much for a premium store. Premium spacing should feel *intentional*, not *vacant*.

13. **Choose the right tier for the section's role, not its perceived importance.** A comparison table or editorial block is `standard` â€” it presents information, not a hero moment. Reserve `spacious` for primary trust-building / CTA sections and `hero` for above-the-fold entries only. When adjacent same-bg sections feel too far apart, first check whether a section is *over-tiered* before touching the gap-same system.
...
15. **Never put spacing on the content element when the CTA is conditionally hidden.** If a CTA (button, link) is visible only on certain breakpoints (e.g. `md:hidden`), do **not** add `lusena-gap-cta` (margin-bottom) to the content element above it â€” that margin will persist even when the CTA is invisible, creating a phantom gap on breakpoints where the CTA is hidden. Instead, put `lusena-gap-cta-top` (margin-top) **on the CTA element itself**. Since the CTA is hidden, its margin disappears with it.

    **Bad:** `<div class="grid lusena-gap-cta">` + `<div class="md:hidden">CTA</div>` â†’ 36px phantom margin on desktop.

    **Good:** `<div class="grid">` + `<div class="md:hidden lusena-gap-cta-top">CTA</div>` â†’ margin only exists on mobile where button is visible.

17. **Dawn's `.section + .section` margin is neutralized â€” but be aware it exists.** Dawn's [base.css](assets/base.css#L149-L157) adds `margin-top: var(--spacing-sections-desktop)` (default 36px) between adjacent `.section` wrappers. Shopify adds the `section` class to the wrapper `<div class="shopify-section section">` when a section schema declares `"class": "section"`. This margin **stacks on top of** LUSENA tier padding, inflating every section gap by ~36px.

    The LUSENA spacing system neutralizes this with a `:has()` rule in `lusena-spacing.css` that zeros the Dawn margin whenever either adjacent section uses a `lusena-spacing--*` tier class. **You don't need to do anything in normal usage** â€” the fix is automatic.
    ...
    **When to be aware:**
    ...
    - If you're mixing LUSENA sections with vanilla Dawn sections on the same page, the margin is zeroed for any pair where at least one side uses LUSENA tiers. Pure Dawnâ†”Dawn pairs keep Dawn's original behavior.
    - The neutralization rule uses CSS `:has()` â€” supported in all modern browsers but not IE11 (not a concern for Shopify stores).
...
18. **Audit same-bg pairs before shipping.** When creating or reordering sections on a page, check whether any adjacent pair shares the same `bg-*` class. If they do, calculate the total visual gap:
    ...
    - **Desktop:** 100â€“140px total (below 100 feels cramped; above 150 feels vacant)
    - **Mobile:** 64â€“90px total

    If the total exceeds these ranges, the likely fix is to *lower the tier* on one or both sections â€” not to tweak the gap-same token.
