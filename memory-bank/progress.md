...
- [x] Cart page CSS extraction (`assets/lusena-cart-page.css`) — 634 lines extracted from cart-items, cart-footer, and cart-quantity `{% stylesheet %}` blocks. Resolved compiled_assets truncation (85KB → ~59KB). (2026-03-26)
...
- [x] Bidirectional cart sync — cart page ↔ drawer sync via `PUB_SUB_EVENTS.cartUpdate` pubsub. Drawer re-renders when cart page changes; cart page re-renders when drawer changes. (2026-03-26)
