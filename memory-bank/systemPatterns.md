...
- **Icons (animated):** `snippets/lusena-icon-animated.liquid` + `assets/lusena-icon-animations.css` — 11 animated SVG icons (heart, layers, droplets, wind, shield-check, sparkles, gift, clock, moon, feather, palette). Each icon has CSS animation classes on sub-elements. Stagger via `--lusena-anim-stagger` custom property. `prefers-reduced-motion` disables all animations. Falls back to static `lusena-icon` for unknown names. Loaded per-section (currently only in `lusena-pdp-feature-highlights`). Animation specs per product documented in `memory-bank/doc/products/{handle}.md`.
- **Icon semantic system (variable cards 1/3/5):** Each icon has one fixed meaning — `heart` (gentle on body), `wind` (cool & gentle), `droplets` (locks moisture in), `sparkles` (radiant & fresh), `moon` (all night/every night), `clock` (fits your routine), `feather` (weightless & traceless), `palette` (your colors). Definitions in `product-setup-checklist.md`.
- **Badge overlay:** `.lusena-badge--overlay` in `lusena-foundations.css` — shared frosted glass badge (white 90% + blur 4px, transparent border). Used by product cards (homepage bestsellers), bundle cards (homepage bundles), and PDP gallery (bestseller label). PDP overrides position/padding for larger gallery context in `lusena-pdp.css`. Replaces 3 former custom implementations (`.lusena-product-card__badge`, `.lusena-bundles__badge`, `.lusena-media-stage__bestseller`).
