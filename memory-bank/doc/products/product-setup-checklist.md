...
| `lusena.pdp_packaging_items` | Opakowanie: Zawartość (skip for bundles - no packaging accordion on bundle template) | List of single-line text |
...
| `lusena.upsell_role` | Upsell: Rola produktu | Single-line text |
| `lusena.upsell_suppress` | Upsell: Ukryj upsell | True or false |
| `lusena.upsell_primary` | Upsell: Główna rekomendacja | Product reference |
| `lusena.upsell_secondary` | Upsell: Zapasowa rekomendacja | Product reference |
| `lusena.upsell_message` | Upsell: Wiadomość | Single-line text |
| `lusena.bundle_original_price` | Bundle: Cena oryginalna (bundles only) | Integer |
| `lusena.bundle_nudge_map` | Bundle: Mapa upsell (bundles only) | JSON |
...
### Per-product upsell metafields

| Metafield | When to set | Example |
|---|---|---|
| `lusena.upsell_role` | Set to `hero` on flagship products (triggers bundle upsell priority). Set to `bundle` on bundle products (suppresses upsell display). | `hero` on poszewka-jedwabna |
| `lusena.upsell_suppress` | Set to `true` to hide upsell for this product | Rarely used |
| `lusena.upsell_primary` | Product reference to the primary upsell recommendation (bundle or cross-sell product) | Link to nocna-rutyna bundle |
| `lusena.upsell_secondary` | Fallback upsell if primary is already in cart | Link to piekny-sen bundle |
| `lusena.upsell_message` | Gain-framed message shown on cross-sell card | "Uzupelnij swoja jedwabna kolekcje" |

### Bundle-only metafields

| Metafield | Purpose | Example |
|---|---|---|
| `lusena.bundle_original_price` | Sum of component prices (integer, e.g., 508 for 508 zl). Renders as crossed-out price. | 508 |
| `lusena.bundle_nudge_map` | JSON mapping trigger handles to upsell data. See schema below. | See below |

**`bundle_nudge_map` JSON schema:**
```json
{
  "trigger-product-handle": {
    "label": "accusative name for headline (Dodaj {label} i zaoszczedz...)",
    "handle": "component-product-handle (resolves title + image via all_products)",
    "tile_label": "optional override for tile display name"
  }
}
```
